	...
	//Student
		...
		public int insertFinalRecords( ) {
			...
			for(Student st:student) {
				...
				try {
					con= Jdbcutil.getMySqlConnection();
					ps = con.prepareStatement("INSERT INTO MasterLists VALUES (?,?,?,?,?)");
					ps.setInt(1,i);
			...
			for(Teachers st:teacher) {
				...
				try {
					con= Jdbcutil.getMySqlConnection();
					ps = con.prepareStatement("UPDATE  MasterLists SET teacher_name=? where id=?");
			...
			for(Classes st:classes) {
				...
				try {
					con= Jdbcutil.getMySqlConnection();
					ps = con.prepareStatement("UPDATE MasterLists SET class_name=? where id=?");
			...
			for(Subject st:subject) {
				...
				try {
					con= Jdbcutil.getMySqlConnection();
					ps = con.prepareStatement("UPDATE MasterLists SET sub_name=? where id=?");
		...
		public boolean getAllRecors() {
			System.out.println("Get Values from Master List");
			System.out.println("");
			System.out.println("");
			System.out.println("*************START-all records()******************");
			System.out.println("");
			try {
				con= Jdbcutil.getMySqlConnection();
				  ps = con.prepareStatement("SELECT * FROM MasterLists");
				  rs=ps.executeQuery();
			...
			System.out.println("*************END-All records******************");
